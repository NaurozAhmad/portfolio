			...
			if (!t.unloads) {
				function unload() {
					...
					if (li) {
						...
						for (n in li) {
							...
							if (o && o.func)
								o.func.call(o.scope, 1); // Send in one arg to distinct unload and users destroy
						}
				...
				function fakeUnload() {
					...
					if (d.readyState == 'interactive') {
						...
						// to execute if the users presses cancel in an onbeforeunload
...
(function() {
	...
	tinymce.create('tinymce.util.URI', {
		URI : function(u, s) {
			...
			u = /^(?:(?![^:@]+:[^:@\/]*@)([^:\/?#.]+):)?(?:\/\/)?((?:(([^:@]*):?([^:@]*))?@)?([^:\/?#]*)(?::(\d*))?)(((\/(?:[^?#](?![^?#\/]*\.[^?#\/.]+(?:[?#]|$)))*\/?)?([^?#\/]*))(?:\?([^#]*))?(?:#(.*))?)/.exec(u);
			each(["source","protocol","authority","userInfo","users","password","host","port","relative","path","directory","file","query","anchor"], function(v, i) {
				var s = u[i];
...
(function(tinymce) {
	...
	function focusBody(ed) {
		...
		// wouldn't get proper focus if the users clicked on the HTML element
...
(function(tinymce) {
	...
	tinymce.html.Schema = function(settings) {
		...
		// If the users didn't allow span only allow internal spans
...
(function(tinymce) {
	...
	tinymce.create('tinymce.dom.DOMUtils', {
			...
			return this.run(e, function(e) {
				...
				switch (n) {
					case "style":
						...
						// No mce_style for elements with these since they might get resized by the users
						if (s.keep_values) {
							if (v && !t._isRes(v))
		...
		split : function(pe, e, re) {
			...
			// but we don't want that in our code since it serves no purpose for the end users
...
(function(tinymce) {
	...
	tinymce.create('tinymce.dom.Selection', {
		...
		getRng : function(w3c) {
			...
			if (t.selectedRange && t.explicitRange) {
				if (r.compareBoundaryPoints(r.START_TO_START, t.selectedRange) === 0 && r.compareBoundaryPoints(r.END_TO_END, t.selectedRange) === 0) {
					...
					// This lets us use the originally set range if the selection hasn't been changed by the users.
					r = t.explicitRange;
				} else {
		...
		_fixIESelection : function() {
			...
			// Detect when users selects outside BODY
...
(function(tinymce) {
	...
	tinymce.create('tinymce.Editor', {
		...
		setupIframe : function() {
			...
			// Register users defined formats
	...
	tinymce.Formatter = function(ed) {
		...
		function apply(name, vars, node) {
			...
			function adjustSelectionToVisibleSelection() {
				...
				// as this isn't visible to the users.
				var rng = ed.selection.getRng();
